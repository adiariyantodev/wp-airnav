...
if (!empty($feeds) && is_array($feeds)) {
    ...
    foreach ($feeds as $index => $feed) {
        if ($index >= $sinceId && $index <= $maxId) {
            ...
            ?>
            <div tabindex="0" role="group" class="wpsr-ig-post <?php echo ($layout_type === 'carousel' && defined('WPSOCIALREVIEWS_PRO')) ? 'swiper-slide' : ''; ?>" data-post_id="<?php echo esc_attr(Arr::get($feed, 'id', ''));?>" data-user_name="<?php echo esc_attr(Arr::get($feed, 'username', ''));?>" data-image_size="<?php echo esc_attr(Arr::get($template_meta, 'post_settings.resolution', 'full'));?>">
