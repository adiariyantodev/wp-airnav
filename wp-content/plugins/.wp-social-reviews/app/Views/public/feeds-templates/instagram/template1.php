...
if (!empty($feeds) && is_array($feeds)) {
    ...
    foreach ($feeds as $index => $feed) {
        if ($index >= $sinceId && $index <= $maxId) {
            ...
            <div role="group" tabindex="0" class="wpsr-ig-post <?php echo ($layout_type === 'carousel' && defined('WPSOCIALREVIEWS_PRO')) ? 'swiper-slide' : ''; ?>" data-user_name="<?php echo esc_attr(Arr::get($feed, 'username', ''));?>" data-post_id="<?php echo esc_attr(Arr::get($feed, 'id', ''));?>" data-image_size="<?php echo esc_attr($imageSize);?>">
                ...
                >
                     ...
                     * */
                    do_action('wpsocialreviews/instagram_post_media', $feed, $template_meta, $index, $display_optimize_image);
                    ?>
                </a>
                <?php if (count($feed) > 6) {
                    ?>
